...
# Load dataset (assuming penguins CSV is available locally)
penguins <- read_csv("penguins.csv", show_col_types = FALSE)

# 5.1 Base R Histogram — Distribution of Body Mass
hist(
...
# 5.2 Scatter Plot — Bill Length vs Flipper Length
scatter_plot <- ggplot(data = penguins, aes(x = bill_length_mm, y = flipper_length_mm)) +
  geom_point(alpha = 0.7, size = 3, colour = "darkblue") +
  geom_smooth(colour = "gray20") +
  labs(
...
# 5.3 Boxplot — Body Mass by Species
box_plot <- ggplot(data = penguins, aes(x = species, y = body_mass_g, fill = species)) +
  geom_boxplot() +
  labs(
...
# 5.4 Faceted Histogram — Flipper Length per Island
facet_plot <- ggplot(data = penguins, aes(x = flipper_length_mm)) +
  geom_histogram(bins = 20, fill = "coral", color = "black") +
  facet_wrap(~ island) +
  labs(
    title = "Distribution of Flipper Length Across Islands",
    x = "Flipper Length (mm)",
    y = "Count"
  )
...
# 5.5 Save Favourite Plot
# Saving the scatter plot as an example
ggsave("my_universe_plot.png", scatter_plot, width = 8, height = 6)
```